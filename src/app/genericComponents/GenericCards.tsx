...
const SmallCard = ({title, number, percentage, variant = "up"}: {title: string, number:string, percentage?:Number, variant:  "up" | "down"}) => {
  ...
  return (
    <Card className="px-6 py-6 w-[24.25%] mr-[1%] last:mr-0 bg-primary">
        <CardTitle className="font-light text-[14px] text-textsecondary-light">{title}</CardTitle>
        <CardDescription className="text-3xl font-bold flex items-center pt-1">
            {number.toString()}
            {percentage &&
            <span className={"ml-2 flex justify-center items-center rounded-xl border px-3 text-[12px] h-[24px] gap-1"+" "+classes}>
                {variant === "up" ? <ArrorUp /> : <ArrorDown />}{percentage.toString()}%
            </span>}
        </CardDescription>
    </Card>
  )
    ...
    return (
        <Card className="p-6 w-[24.25%] mr-[1%] last:mr-0 bg-primary">
            ...
            <CardDescription className="text-3xl font-bold flex items-center pt-2">
                ...
                <span className={"ml-2 flex justify-center items-center rounded-xl border  text-[12px] h-[20px] gap-1"}> 
...
const SingleBarCard = ({title, graphData}: {title:string, graphData: {title:string; percentage:number;}[]}) => {

    return (
        <Card className="px-1 pt-3 pb-5 w-[24.25%] mr-[1%] last:mr-0 bg-primary h-[50vh] ">
            <CardTitle className="font-light text-[14px] text-textsecondary-light mb-2 pt-2 px-3">{title}</CardTitle>
            <div className="flex flex-col px-3 justify-evenly w-full p-0 overflow-scroll h-[92%] bg-primary">
            {graphData.map((data, index) => (
                <SingleBar key={index} title={data.title} percentage={data.percentage}/>
            ))}
            </div>
        </Card>
    )
    ...
    return (
        <Card className="px-6 py-3 w-[24.25%] mr-[1%] last:mr-0 bg-primary h-[50vh]">
            ...
            {[...Array(numberOfBars)].map((_, index) => (
                <div key={index}>
                    <div className={"w-full mt-2 h-4 rounded-[6px] overflow-hidden bg-secondary"}>
